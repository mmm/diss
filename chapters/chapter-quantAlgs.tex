...
This transformation preserves $\ket{s}$, but flips 
the sign of any vector orthogonal to $\ket{s}$.
...
\end{split}
\end{equation}
Since the ket $\ket\omega$ was chosen an element of the orthonormal
basis $\ket{i}$,
\begin{equation}
\sum_j \braket{j}{\omega} = 1,\qquad\text{and}\qquad
\sum_j \braket{j}{\omega_\perp} = N-1.
\end{equation}
Then, a single step of Grover looks like
\begin{equation}
{\bf U}_{\text{Grover}}\ket\Psi
\quad =\quad 
                k\ket{\omega} - l\ket{\omega_\perp}
                + \frac{2}{N} \sum_i \ket{i} \left(
                              ...
                              \right).
\end{equation}
To find the coefficients for the state after this iteration,
take
\begin{equation}
\begin{split}
...
\mmm{add in discrete sine and cosine stuff}
